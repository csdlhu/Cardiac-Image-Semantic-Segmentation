...
class ModelBuilder():
    ...
    def build_unet(self, num_class=1, arch='albunet', weights=''):
        arch = arch.lower()
        unet = CNN(num_classes=num_class)
...
class CNN(nn.Module):
    def __init__(self, num_classes=4, num_filters=32, pretrained=True, is_deconv=True):
        ...
        # Decoder
        self.center = PSPModule(features=1024, norm_layer=nn.BatchNorm2d, out_features=num_filters * 16)
    ...
    def forward(self, x):
        ...
        # no dfp
        dec3, _ = self.dec3([dec4, m3])
        dec2, _ = self.dec2([dec3, m2])
        dec1 = self.dec1(dec2)
